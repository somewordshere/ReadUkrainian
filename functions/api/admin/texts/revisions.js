...
import { listTextRevisions, storyExists } from "../../../_shared/texts.js";
...
export async function onRequestGet(context) {
  ...
  const [exists, revisions] = await Promise.all([
    storyExists(context.env.DB, storyId),
    listTextRevisions(context.env.DB, storyId),
  ]);
  if (!exists) return error(404, "Story not found.");
