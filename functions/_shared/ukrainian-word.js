...
export function canonicalizeUkrainianWord(value, { allowSurroundingPunctuation = true } = {}) {
  const normalized = normalizeUkrainianText(value);

  if (!allowSurroundingPunctuation) {
    return UKRAINIAN_WORD_PATTERN.test(normalized) ? normalized : null;
  }

  return normalized.match(SELECTED_WORD_PATTERN)?.groups?.word || null;
}

export function extractUkrainianWords(value) {
  const words = [];

  for (const match of normalizeUkrainianText(value).matchAll(STORY_TOKEN_PATTERN)) {
    if (UKRAINIAN_WORD_PATTERN.test(match[0])) {
      words.push(match[0]);
    }
  }
