...
const lines=["-- Run only after release 0.83 has been applied. Stops if an editor changed affected content.","-- New stories are hidden, not deleted. Drafts, revisions and dictionary additions remain.","CREATE TABLE _a1_083_rollback_guard (ok INTEGER NOT NULL CHECK(ok=1));",`INSERT INTO _a1_083_rollback_guard SELECT CASE WHEN ${guards.join(" AND ")} THEN 1 ELSE 0 END;`,"DROP TABLE _a1_083_rollback_guard;"];
