...
for(const [word,wanted] of expected){
  ...
  for(let attempt=0;attempt<3;attempt++) {
    const response=await fetch(`${origin}/api/dictionary/lookup`,{method:"POST",headers:{"content-type":"application/json",origin},body:JSON.stringify({text:word,targetLanguage:"en"}),signal:AbortSignal.timeout(20000)});
